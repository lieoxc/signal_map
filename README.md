...
为了支持多设备的H3网格信号系统，设计了极简的数据库表结构：

**核心表：**
1. **h3_grid_data** - H3网格数据表（存储H3聚合数据）

**主要特性：**
- 支持多设备管理（通过device_id字段）
- 支持多种H3分辨率（通过resolution字段）
- 优化的索引设计
- 极简的表结构，易于维护

**表结构说明：**
- `device_id`: 设备唯一标识符（如：8UUDMAQ00A0202）
- `resolution`: H3分辨率级别（0-15）
- `h3_index`: H3网格索引（全球唯一）
- `point_count`: 网格内数据点数量
- `avg_sdr_signal`: 平均SDR信号强度
- `max_sdr_signal`: 最大SDR信号强度
- `min_sdr_signal`: 最小SDR信号强度

**使用方式：**
- 每次处理飞行记录时，将H3聚合数据直接插入此表
- API查询时直接从该表获取数据返回给前端
- 支持按设备、分辨率、H3索引等条件查询

详细表结构请参考：`database_schema.sql`
